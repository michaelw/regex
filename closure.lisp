...
; returns (values matchfn initfns linkfns)
   ...
   #'(lambda (pos)
       ...
       (let ((reg-str-end (+ pos len)))
         ...
         (when (<= reg-str-end *end*)
           (loop for i from pos below reg-str-end
                 for j from 0 below len
       ...
       (let ((reg-start (register-start *regs* regnum))
         ...
         (cond ((numberp reg-start)
                ...
                (let* ((reg-len (- reg-end reg-start))
                  ...
                  (when (<= reg-str-end *end*)
                    (loop for i from pos below reg-str-end
                          for j from reg-start below reg-end
   ...
   #'(lambda (pos)
       ...
       (when (< pos *end*)
         (let ((chr (re-char *str* pos)))
           (declare (character chr))
           (loop for discriminator-idx from 0 by 2 below (* 2 num-jump-entries)
