                            ...
                            (if *allow-backmatch*
                                (let ((item (scan-num scanner)))
                                  (if (numberp item)
                                    ...
                                    (values 'char (fix-escape-char item))))
                              (values 'char (nextchar scanner)))
                          (values 'char ch)))
                       (#\*
          ...
          (unless (eq token3 'rparen)
            (throw 'regex-parse-error
                   (list "parse-group: Expected ')' at token ~S ~S"
                         token3 value3)))
          (cond ((numberp value) (make-register-node value regex))
                (t regex))))))
   ((member token '(any startanchor endanchor char backmatch startclass))
